            ...
            <div class="card-body">
                <form class="row g-3" action="{{ route('tournament-management.update', $tournament->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" name="name" class="form-control @error('name') is-invalid @enderror" id="name" value="{{ old('name', $tournament->name) }}" placeholder="Enter tournament name" required>
                        @error('name')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="date" name="start_date" class="form-control @error('start_date') is-invalid @enderror" id="start_date" value="{{ old('start_date', \Carbon\Carbon::parse($tournament->start_date)->format('Y-m-d')) }}" required>
                        @error('start_date')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="col-md-6">
                        ...
                        <input type="date" name="end_date" class="form-control @error('end_date') is-invalid @enderror" id="end_date" value="{{ old('end_date', $tournament->end_date ? \Carbon\Carbon::parse($tournament->end_date)->format('Y-m-d') : '') }}">
                        @error('end_date')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="col-md-6">
                        ...
                        <input type="text" name="location" class="form-control @error('location') is-invalid @enderror" id="location" value="{{ old('location', $tournament->location) }}" placeholder="Enter location" required>
                        @error('location')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="col-md-6">
                        ...
                        <select name="status" class="form-control @error('status') is-invalid @enderror" id="status">
                            ...
                            <option value="{{ $status->value }}" {{ old('status', $tournament->status->value) == $status->value ? 'selected' : '' }}>{{ \Illuminate\Support\Str::title($status->value) }}</option>
                            @endforeach
                        </select>
                        @error('status')
                        <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="number" step="0.01" min="0" max="100" name="tax_rate" class="form-control @error('tax_rate') is-invalid @enderror" id="tax_rate" value="{{ old('tax_rate', $tournament->tax_rate) }}" placeholder="e.g. 7.5">
                        @error('tax_rate')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                        ...
                        <div class="table-responsive">
                            <table class="table table-sm">
                                ...
                                <tbody>
                                    ...
                                    @foreach(($items ?? []) as $it)
                                    @php $attached = $itemPivot->has($it->id); $override = $attached ? $itemPivot[$it->id]->pivot->price : null; @endphp
                                    <tr>
                                        <td>
                                        ...
                                        <td>
                                            <input type="number" step="0.01" min="0" name="items[{{ $it->id }}][price]" class="form-control form-control-sm" placeholder="Leave blank for default" value="{{ $override }}">
                                        </td>
                                    </tr>
                                    @endforeach
                                </tbody>
                        ...
                        <div class="table-responsive">
                            <table class="table table-sm">
                                ...
                                <tbody>
                                    ...
                                    @php $attached = $bundlePivot->has($b->id); $override = $attached ? $bundlePivot[$b->id]->pivot->price : null; @endphp
                                    <tr>
                                        <td>
                                        ...
                                        <td>
                                            <input type="number" step="0.01" min="0" name="bundles[{{ $b->id }}][price]" class="form-control form-control-sm" placeholder="Leave blank for default" value="{{ $override }}">
                                        </td>
                                    </tr>
                                    @endforeach
                                </tbody>
