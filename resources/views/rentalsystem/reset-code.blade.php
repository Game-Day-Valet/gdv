...
<head>
    ...
    <title>Verify Reset Code - Game Day Valet</title>
...
<body>
    ...
    <main class="page">
        ...
        <section>
            <div class="card">
                <h2>Verify code</h2>
                <p class="sub">Enter the code to set a new password</p>
                ...
                <form method="POST" action="{{ route('rentalsystem.reset-password.code.submit') }}">
                    @csrf
                    <input type="hidden" name="email" value="{{ $email }}">
                    <label class="label">VERIFICATION CODE</label>
                    <div class="otp">
                        <input class="input" name="d1" maxlength="1" pattern="[0-9]*" inputmode="numeric" required>
                        <input class="input" name="d2" maxlength="1" pattern="[0-9]*" inputmode="numeric" required>
                        <input class="input" name="d3" maxlength="1" pattern="[0-9]*" inputmode="numeric" required>
                        <input class="input" name="d4" maxlength="1" pattern="[0-9]*" inputmode="numeric" required>
                        <input class="input" name="d5" maxlength="1" pattern="[0-9]*" inputmode="numeric" required>
                        <input class="input" name="d6" maxlength="1" pattern="[0-9]*" inputmode="numeric" required>
                    </div>
    ...
    <script>
        ...
        inputs.forEach((input, idx) => {
            input.addEventListener('input', function() {
                ...
                document.getElementById('code').value = Array.from(inputs).map(i => i.value).join('');
            });
